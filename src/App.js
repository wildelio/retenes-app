...
function MapController({ flyTo }) {
  const map = useMap();
  useEffect(() => {
    if (flyTo) map.flyTo(flyTo, 16, { duration: 1.2 });
  }, [flyTo, map]);
  return null;
}
...
function Buscador({ onFlyTo }) {
  ...
  const buscar = async (texto) => {
    ...
    try {
      const res = await fetch(
        `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(texto + ' Matamoros Tamaulipas')}&format=json&limit=5&countrycodes=mx`,
  ...
  const handleChange = (e) => {
    ...
    timeoutRef.current = setTimeout(() => buscar(e.target.value), 500);
  };
  ...
  return (
    <div className="buscador-wrapper">
      <div className="buscador-input-row">
        ...
        <input
          type="text"
          placeholder="Buscar calle o colonia en Matamoros..."
          value={query}
      ...
      {resultados.length > 0 && (
        <ul className="buscador-resultados">
          {resultados.map((r) => (
            <li key={r.place_id} onClick={() => seleccionar(r)}>
...
export default function App() {
  ...
  const [mapCenter] = useState([25.8691, -97.5027]);
      ...
      <header className="header">
        <div className="header-content">
          <div className="logo">
            ...
            <div>
              <h1>RetenApp</h1>
              <span className="tagline">Matamoros, Tamaulipas · En tiempo real</span>
            </div>
          </div>
          <div className="stats">
      ...
      <div className="map-wrapper">
        <MapContainer center={mapCenter} zoom={13} className="map">
          ...
          {reportes.map(r => {
            ...
            return (
              <Marker key={r.id} position={[r.lat, r.lng]} icon={r.confirmaciones >= 3 ? RETEN_ICON_HOT : RETEN_ICON}>
                <Popup className="custom-popup" maxWidth={300}>
                  <div className="popup-content">
                    ...
                    <div className="popup-meta">
                      ...
                      <button className={`btn-confirm ${votos.includes(userToken) ? 'voted' : ''}`} onClick={() => confirmar(r)}>
